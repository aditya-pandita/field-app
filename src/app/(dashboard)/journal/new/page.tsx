      ...
      <form onSubmit={handleSubmit} className="max-w-[600px]">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 mb-8">
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) =>
                setFormData({ ...formData, weather: e.target.value as any })
              }
              className="w-full bg-[#111111] border border-[#252525] text-white px-4 py-3 outline-none focus:border-[#4A4A4A] transition-colors"
            >
              {weather.map((w) => (
          ...
          <div className="flex flex-wrap gap-2">
            {moods.map((mood) => (
              <button
                ...
                className={`px-4 py-2 font-['JetBrains_Mono'] text-[10px] tracking-[2px] uppercase border transition-colors ${
        ...
        <div className="flex gap-4">
          <button
            type="button"
            onClick={() => router.back()}
            className="bg-transparent border border-[#333333] text-[#888888] font-['JetBrains_Mono'] text-[10px] tracking-[3px] uppercase px-7 py-3 cursor-pointer transition-all duration-150 hover:border-[#4A4A4A] hover:text-white hover:bg-[#111111]"
          >
            Cancel
          </button>
          <button
            type="submit"
            disabled={loading}
            className="bg-[#FF5500] text-white font-['JetBrains_Mono'] text-[10px] tracking-[3px] uppercase px-7 py-3 cursor-pointer transition-all duration-150 hover:bg-[#E64D00] disabled:opacity-50"
          >
